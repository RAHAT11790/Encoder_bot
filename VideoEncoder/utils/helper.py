...
async def handle_encode(filepath, message, msg, task_id=None, overrides=None):
    # Deferred import: encoding.py pulls in ..svcs.task_manager, which
    # initializes the whole svcs package. If anything in that package ever
    # imports handle_encode back (as encode_svc.py legitimately needs to),
    # importing encoding.py at module level here would create a circular
    # import. Importing it lazily, on first call instead of at module load,
    # makes this permanently safe regardless of what svcs/__init__.py does.
    ...
    if not res or not res[0]:
        reason = getattr(message, 'encode_fail_reason', None)
        detail = f"\n\n<i>{reason}</i>" if reason else ""
        await message.reply(f"▸ <b>Error</b>{id_str}\nStatus: ✗ Encoding Failed{detail}")
        return None
...
async def handle_extract(archieve):
    ...
    extract_to = await get_zip_folder(archieve)

    lower = archieve.lower()

    # Formats the Python stdlib can handle directly -- no external binary.
    if lower.endswith(".zip"):
        import zipfile
        with zipfile.ZipFile(archieve) as zf:
            zf.extractall(extract_to)
    elif any(lower.endswith(sfx) for sfx in
             (".tar.gz", ".tar.bz2", ".tar.xz", ".tgz", ".tbz2", ".tar", ".gz", ".bz2", ".xz")):
        import tarfile
        mode = "r:gz" if lower.endswith(".gz") or lower.endswith(".tgz") else (
            "r:bz2" if lower.endswith(".bz2") or lower.endswith(".tbz2") else (
            "r:xz" if lower.endswith(".xz") else "r:"))
        with tarfile.open(archieve, mode) as tf:
            tf.extractall(extract_to)
    else:
        # Everything else (7z, rar, iso, ...) needs an external tool.
        tool = None
        ...
        if not tool:
            raise RuntimeError(
                "No extractor available for this archive. Please install p7zip "
                "(e.g. `apt-get install -y p7zip-full`)."
            )
        rio = await asyncio.create_subprocess_exec(
            tool, "x", f"-o{extract_to}", "-y", archieve,
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
        await rio.communicate()
        if rio.returncode != 0:
            raise RuntimeError(f"Extraction failed (7z exit code {rio.returncode}).")

    if os.path.exists(archieve):
        os.remove(archieve)
    return extract_to
